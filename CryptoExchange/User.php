...
class User
{
    ...
    public function __construct(string $name, Wallet $wallet, array $portfolio = [], int $id = 0)
    {
        $this->name = $name;
        $this->wallet = $wallet;
    ...
    public function buyCryptocurrency(Cryptocurrency $crypto, float $amount): void
    {
        ...
        $stmt = $pdo->prepare("INSERT INTO portfolio (user_id, symbol, amount, price, last_updated) VALUES (:user_id, :symbol, :amount, :price, :last_updated)");
        $stmt->execute([
            ':user_id' => $this->id,
            ':symbol' => $symbol,
            ':amount' => $amount,
    ...
    public function sellCryptocurrency(Cryptocurrency $crypto, float $amount): void
    {
        ...
        $pdo = Database::getInstance()->getConnection();
        $stmt = $pdo->prepare("DELETE FROM portfolio WHERE user_id = :user_id AND symbol = :symbol AND amount <= :amount");
        $stmt->execute([
            ':user_id' => $this->id,
            ':symbol' => $symbol,
            ':amount' => $amount
    ...
    public static function loadUsers(): array
    {
        ...
        foreach ($data as $userData) {
            ...
            $portfolio = User::loadPortfolio($userData['id']);
            $users[] = new self($userData['name'], $wallet, $portfolio, $userData['id']);
        }
        return $users;
    }
    ...
    public static function saveUser(User $user): void
    {
        $pdo = Database::getInstance()->getConnection();
        if ($user->getId() === 0) {
            $stmt = $pdo->prepare("INSERT INTO users (name, wallet_balance) VALUES (:name, :wallet_balance)");
            $stmt->execute([
                ':name' => $user->getName(),
            ...
            $stmt = $pdo->prepare("UPDATE users SET wallet_balance = :wallet_balance WHERE id = :id");
            $stmt->execute([
                ':wallet_balance' => $user->getWallet()->getBalance(),
                ':id' => $user->getId()
            ]);
    ...
    {
        $pdo = Database::getInstance()->getConnection();
        $stmt = $pdo->prepare("SELECT * FROM portfolio WHERE user_id = :user_id");
